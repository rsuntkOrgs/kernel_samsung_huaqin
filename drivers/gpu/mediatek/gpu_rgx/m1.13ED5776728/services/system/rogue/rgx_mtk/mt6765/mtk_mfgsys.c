...
static IMG_BOOL MTKDoGpuDVFS(IMG_UINT32 ui32NewFreqID, IMG_BOOL bIdleDevice)
{
	...
	if (MTKGetRGXDevIdx() == MTK_RGX_DEVICE_INDEX_INVALID)
		return IMG_FALSE;

	eResult = PVRSRVDevicePreClockSpeedChange(MTKGetRGXDevIdx(),
	...
	if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
		...
		MTKWriteBackFreqToRGX(MTKGetRGXDevIdx(), ui32GPUFreq);
	...
	if (gpu_debug_enable)
		...
		if (eResult == PVRSRV_OK)
			PVRSRVDevicePostClockSpeedChange(MTKGetRGXDevIdx(),
			bIdleDevice, (void *)NULL);
